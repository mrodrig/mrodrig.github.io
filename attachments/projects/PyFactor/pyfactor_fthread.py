...
def main():
	...
	for i in range(MIN, MAX): # calculate some factors already so results are quicker for user
		t = threading.Thread(target = findFactors, args = (i,))
		t.daemon = True
		t.start()
		#findFactors(i) # do nothing with results, findFactors memoizes them for us
	print "0%   ###################################################################   100%"
	print "                           Initialization Completed                            "
	print ":: Input a number at the '>' prompt                                            "
	...
	print "      Default largest factored number: " + str(MAX)
	print ":: Type 'save' to save all memoized factors in a file in the current directory "
	print "      for later loading which would prevent having to re-compute factors.      "
	print ":: Type 'load' to load all memoized factors in a previously saved file in the  "
	print "      current directory for which would prevent having to re-compute factors.  "
	...
	while(True):
		try:
			...
			if (inputarg == "exit\n" or inputarg == "quit\n"): # Exit or Quit?
			 	os.system("clear #Clear the Screen")
				exit(0)
#			elif (inputarg.startswith("range save")):
#				args = inputarg.split()
#				if (len(args) > 2 and int(args[2]) > MAX): # Number > MAX
#					for i in range(MAX, int(args[2]) + 1):
#						if (i % 10000 == 0):
#							findFactors(i)
...
#								""
#							else:
#								memo[fakeKey] = "true" # Add fake key and value so we can
#													   # determine whether a loaded dict is
#													   # valid later on or whether it is
#													   # even compatible
#							pickle.dump(memo, save_file)
#							save_file.close()
#							print "File saved at: " + str(i)
#						findFactors(i)
#					MAX = int(args[2])
#				else:
#					print "Invalid input number: " + str(args[2])
				...
				if (len(args) > 1 and int(args[1]) > MAX): # Number > MAX
					...
					print "Invalid input number: " + str(args[1])
				...
				try: # Try converting input to int
				...
				except ValueError: # Can't convert input to an int
					print "Invalid command or number. Please try again."
